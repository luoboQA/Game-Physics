...
PhysicsWorld::~PhysicsWorld() {
    delete m_broadphase;
    for (RigidBody* body : m_bodies) delete body;
}
...
static bool BoxBoxCollision(BoxCollider* boxA, BoxCollider* boxB, ContactManifold* manifold) {
    ...
    Vec3 axesA[3] = { rotA * Vec3(1,0,0), rotA * Vec3(0,1,0), rotA * Vec3(0,0,1) };
    Vec3 axesB[3] = { rotB * Vec3(1,0,0), rotB * Vec3(0,1,0), rotB * Vec3(0,0,1) };
    ...
    if ((centerB - centerA).Dot(bestAxis) < 0) bestAxis = -bestAxis;
...
void PhysicsWorld::GenerateConstraints() {
    ...
    for (const auto& pair : pairs) {
        ...
        if (GJK(colliderA, colliderB, &gjkResult) && gjkResult.isColliding) {
            ...
            if (success) {
                ...
                if (m_enableWarmStarting) {
                    TryMatchPreviousConstraint(constraint, manifold);
                }
    ...
    m_previousConstraints = newConstraints;
    m_constraints = std::move(newConstraints);
}

void PhysicsWorld::TryMatchPreviousConstraint(ContactConstraint& newConstraint, 
                                               const ContactManifold& manifold) {
    const float PERSISTENT_THRESHOLD_SQ = 0.01f;
    for (auto& oldConstraint : m_previousConstraints) {
        if (oldConstraint.GetBodyA() == newConstraint.GetBodyA() &&
            oldConstraint.GetBodyB() == newConstraint.GetBodyB()) {
            Vec3 deltaA = manifold.contactPointA - oldConstraint.GetContactPointA();
            Vec3 deltaB = manifold.contactPointB - oldConstraint.GetContactPointB();
            if (deltaA.LengthSq() < PERSISTENT_THRESHOLD_SQ &&
                deltaB.LengthSq() < PERSISTENT_THRESHOLD_SQ) {
                newConstraint.SetPersistent(true);
                newConstraint.SetAccumulatedImpulse(
                    oldConstraint.GetNormalImpulse(),
                    oldConstraint.GetTangentImpulse1(),
                    oldConstraint.GetTangentImpulse2()
                );
                break;
            }
        }
...
void PhysicsWorld::SolveConstraints(float dt) {
    ...
    if (m_enableWarmStarting) {
        for (auto& constraint : m_constraints) {
            if (constraint.IsPersistent())
                constraint.ApplyWarmStartImpulses(m_warmStartFactor);
            else
    ...
    for (int iteration = 0; iteration < m_numIterations; ++iteration) {
        for (auto& constraint : m_constraints)
            constraint.Solve(dt, m_restitutionSlop);
    }
}

void PhysicsWorld::ApplyPositionalCorrection() {
    for (auto& constraint : m_constraints)
        constraint.PositionalCorrection(m_penetrationSlop);
}
...
void PhysicsWorld::Step(float dt) {
    ...
    for (RigidBody* body : m_bodies) body->Integrate(dt);
    ...
    // 地面修正 - 修正为正确的地面高度
    const float GROUND_TOP = -4.5f;
    for (RigidBody* body : m_bodies) {
        if (!body->m_isStatic) {
            float bottom = body->m_position.y - 0.5f;
            if (bottom < GROUND_TOP) {
                body->m_position.y = GROUND_TOP + 0.5f;
                body->UpdateGlobalCentroidFromPosition();
