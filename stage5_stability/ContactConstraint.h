...
public:
    ...
    // 求解约束（计算并应用冲量）
    void Solve(float dt, float restitutionSlop);
    
    // 重置累积冲量
    void ResetAccumulatedImpulse();
    
    // 热启动：应用上一帧的累积冲量
    void ApplyWarmStartImpulses(float factor);
    
    // 位置修正（防止穿透）
    void PositionalCorrection(float penetrationSlop);
    ...
    // 获取累积冲量
    float GetNormalImpulse() const { return m_accumulatedNormalImpulse; }
    float GetTangentImpulse1() const { return m_accumulatedTangentImpulse1; }
    float GetTangentImpulse2() const { return m_accumulatedTangentImpulse2; }
    
    // 设置累积冲量（用于热启动）
    void SetAccumulatedImpulse(float normal, float tangent1, float tangent2) {
        m_accumulatedNormalImpulse = normal;
        m_accumulatedTangentImpulse1 = tangent1;
        m_accumulatedTangentImpulse2 = tangent2;
    }
    
    // 更新接触点位置
    void SetContactPoints(const Vec3& worldA, const Vec3& worldB,
                          const Vec3& localA, const Vec3& localB) {
        m_contactPointA = worldA;
        m_contactPointB = worldB;
        m_localContactA = localA;
        m_localContactB = localB;
    }
